...
public:
    semaphore()
    {
        h_ = rl_CreateSemaphore(0, 0, LONG_MAX, 0);
    }

    ~semaphore()
    {
        rl::rl_CloseHandle(h_, $);
    }

    void wait()
    {
        rl::rl_WaitForSingleObject(h_, rl::rl_INFINITE, $);
    }

    void post()
    {
        rl::rl_ReleaseSemaphore(h_, 1, 0, $);
    }
...
public:
    mutex()
    {
        rl::rl_InitializeCriticalSection(&cs_, $);
    }

    ~mutex()
    {
        rl::rl_DeleteCriticalSection(&cs_, $);
    }

    void lock()
    {
        rl::rl_EnterCriticalSection(&cs_, $);
    }

    void unlock()
    {
        rl::rl_LeaveCriticalSection(&cs_, $);
    }

private:
    rl::rl_CRITICAL_SECTION    cs_;
...
public:
    ...
    void push(node* n)
    {
        size_t s = size_($).load(rl::mo_relaxed);
        size_($).store(s + 1, rl::mo_relaxed);
        n->next_($) = head_.next_($);
        n->prev_($) = &head_;
        head_.next_($)->prev_($) = n;
        head_.next_($) = n;
    ...
    size_t size() const
    {
        return size_($).load(rl::mo_relaxed);
    }
    ...
    {
        if (size_($).load(rl::mo_relaxed))
        {
            target.size_($).store(size_($).load(rl::mo_relaxed));
...
public:
    ...
    void prepare_wait(ec_thread* th = 0, void* ctx = 0)
    {
        ...
        th->in_waitset_($).store(true, rl::mo_relaxed);
        th->ctx_($) = ctx;
        {
            lock l (mtx_);
            th->epoch_($) = epoch_($).load(rl::mo_relaxed);
    ...
    void commit_wait(ec_thread* th = 0)
    {
        ...
        //if (th->epoch_($) == epoch_($).load(rl::mo_relaxed))
        if (th->in_waitset_($).load(rl::mo_acquire))
            th->sema_.wait();
        else
    ...
    void cancel_wait(bool /*from_commit*/, ec_thread* th = 0)
    {
        ...
        {
            lock l (mtx_);
            if (th->in_waitset_($).load(rl::mo_relaxed))
            {
                // successfully removed from waitset,
                // so there will be no spurious wakeup
                th->in_waitset_($).store(false, rl::mo_relaxed);
                th->spurious_($) = false;
    ...
    void notify_one_relaxed()
    {
        ...
        {
            lock l (mtx_);
            unsigned ep = epoch_($).load(rl::mo_relaxed);
            epoch_($).store(ep + 1, rl::mo_relaxed);
            n = waitset_.pop();
            if (n)
                to_ec_thread(n)->in_waitset_($).store(false, rl::mo_release);
        }
        if (n)
        {
        ...
        dlist temp;
        {
            ...
            while (dlist::not_last(n))
            {
                ...
                if (pred(th->ctx_($), size, idx))
                {
                    waitset_.remove(n);
                    temp.push(n);
                    th->in_waitset_($).store(false, rl::mo_release);
                }
                n = next;
                idx += 1;
            }
...
struct queue
{
    ...
    void enqueue(void* data)
    {
        ...
        buffer_[idx]($).store(data, rl::mo_relaxed);
    ...
    void* dequeue(ec_thread* th)
    {
        ...
        void* data = buffer_[idx]($).load(rl::mo_relaxed);
        if (data)
            return data;
        for (;;)
        {
            ...
            data = buffer_[idx]($).load(rl::mo_relaxed);
            if (data)
            {
                return data;
            }
