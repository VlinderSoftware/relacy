...
public:
	...
	~ws_deque()
	{
		...
		{
			next = p->header_.next_($).load(rl::mo_relaxed);
            delete p;
		}
	}
    ...
    void push_unbalanced(void* i)
    {
		...
		bottom_.block_($)->data_[bottom_.real_index_]($).store(i, rl::mo_release);
    ...
    void* pop_unbalanced()
    {
		...
		{
			bottom_.real_index_ -= 1;
			void* i = bottom_.block_($)->data_[bottom_.real_index_]($).load(rl::mo_consume);
			return i;
		}
		else
		{
...
retry:
		for (;;)
		{
			block* old_b = top_.block_($).load(rl::mo_acquire);
		    block* b = old_b;
			top::info old = top_.info_($).load(rl::mo_consume);
			...
			if (b->header_.id_ != old.top_block_id_)
			{
				do
				{
					b = b->header_.next_($).load(rl::mo_relaxed);
                    //RL_ASSERT(b);
				...
				if (top_.block_($).compare_exchange_strong(old_b, b, rl::mo_seq_cst))
				{
					block* cur_b = old_b;
					do
					{
						pdr_defer(cur_b);
						cur_b = cur_b->header_.next_($).load(rl::mo_relaxed);
					}
					while (cur_b != b);
				}
			}
			...
			void* i = b->data_[mod.top_index_]($).load(rl::mo_consume);

			if (block::item_count - 1 == mod.top_index_)
			{
				next_block = b->header_.next_($).load(rl::mo_relaxed);
				mod.top_block_id_ += 1;
				mod.top_index_ = 0;
			}
			else
			...
			if (top_.info_($).compare_exchange_strong(old, mod, rl::mo_seq_cst))
			{
				if (next_block)
				{
					if (top_.block_($).compare_exchange_strong(b, next_block))
					{
						pdr_defer(b);
					}
				}
	...
	unsigned size() const
	{
		top::info const top = top_.info_($).load(rl::mo_relaxed);
...
private:
	struct block
	{
		struct header
		{
			rl::atomic<block*> next_;
			rl::atomic<block*> prev_;
			ws_deque* deque_;
			unsigned id_;
		...
		header header_;
		rl::atomic<void*> data_[item_count];
	};
	...
	struct top
	{
		...
		rl::atomic<block*> block_;
		rl::atomic<info> info_;
	};
	...
	void alloc_block()
	{
		...
		block* b = bottom_.block_($) ? bottom_.block_($)->header_.next_($).load(rl::mo_relaxed) : 0;
		if (0 == b)
		{
			b = new block;
			...
			b->header_.prev_($).store(bottom_.block_($), rl::mo_relaxed);
			if (bottom_.block_($))
				bottom_.block_($)->header_.next_($).store(b, rl::mo_relaxed);
			b->header_.next_($).store(0, rl::mo_relaxed);
		}
		else
		{
            b = b;
	...
	pop_check_result pop_check(void*& i)
	{
		...
		if (bottom_.block_id_ == bottom_.real_block_id_
			&& bottom_.index_ == bottom_.real_index_)
		{
			top::info const top = top_.info_($).load(rl::mo_seq_cst);
	...
	void* pop_slow()
	{
		bottom_.block_seq_ -= 1;
		bottom_.block_seq_ &= 0xffff;
		bottom_.block_($) = bottom_.block_($)->header_.prev_($).load(rl::mo_relaxed);
		...
		top::info i = top_.info_($).load(rl::mo_relaxed);
		...
		void* v = bottom_.block_($)->data_[block::item_count - 1]($).load(rl::mo_consume);
        return v;
	}
	...
	void check_bottom()
	{
		...
		for (;;)
		{
			top::info old = top_.info_($).load(rl::mo_relaxed);
			...
			*/
			...
			if (top_.info_($).compare_exchange_strong(old, i, rl::mo_seq_cst))
			{
